...
contract SideBridge {
    ...
    event TokensBridged(address indexed requester, uint indexed mainDepositHash, uint amount, uint timestamp);
    event TokensReturned(address indexed requester, uint indexed sideDepositHash, uint amount, uint timestamp);
    ...
    function bridgeTokens (address _requester, uint _bridgedAmount, uint _mainDepositHash) verifyInitialization onlyGateway  external {
        sideToken.mint(_requester,_bridgedAmount);
        emit TokensBridged(_requester, _mainDepositHash, _bridgedAmount, block.timestamp);
    }

    function returnTokens (address _requester, uint _bridgedAmount, uint _sideDepositHash) verifyInitialization onlyGateway external {
