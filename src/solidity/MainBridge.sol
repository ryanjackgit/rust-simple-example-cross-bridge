...
contract MainBridge {
    ...
    event TokensLocked(address indexed requester, uint indexed mainDepositHash, uint amount, uint timestamp);
    event TokensUnlocked(address indexed requester, uint indexed sideDepositHash, uint amount, uint timestamp);
    ...
    function lockTokens (address _requester, uint _bridgedAmount, uint _mainDepositHash) onlyGateway external {
        emit TokensLocked(_requester, _mainDepositHash, _bridgedAmount, block.timestamp);
    }

    function unlockTokens (address _requester, uint _bridgedAmount, uint _sideDepositHash) onlyGateway external {
